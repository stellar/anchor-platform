...
class RpcPlatformApiTests : PlatformApiTests() {
  ...
  @Test
  @Order(35)
  fun `concurrent RPC calls on same transaction should only allow one to succeed`() {
    // Create a SEP-24 deposit transaction
    val depositRequest = gson.fromJson(RPC_DEPOSIT_REQUEST, HashMap::class.java)
    ...
    // Build the same RPC request (request_offchain_funds) to fire concurrently
    val requestOffchainFundsParams =
      gson.fromJson(REQUEST_OFFCHAIN_FUNDS_PARAMS, RequestOffchainFundsRequest::class.java)
    requestOffchainFundsParams.transactionId = txId
    ...
    val futures =
      (1..concurrency).map {
        executor.submit<okhttp3.Response> {
          val rpcRequest =
            RpcRequest.builder()
              .id(it)
              .method(REQUEST_OFFCHAIN_FUNDS.toString())
              ...
              .params(requestOffchainFundsParams)
              .build()
          platformApiClient.sendRpcRequest(listOf(rpcRequest))
        }
      }
    ...
    // Parse responses and count successes vs errors
    val rpcResponseType =
      object : com.google.gson.reflect.TypeToken<List<org.stellar.anchor.api.rpc.RpcResponse>>() {}
        .type
    var successCount = 0
    var errorCount = 0
    for (response in responses) {
      val body = response.body?.string() ?: "[]"
      val rpcResponses: List<org.stellar.anchor.api.rpc.RpcResponse> =
        gson.fromJson(body, rpcResponseType)
    ...
    // Exactly one should succeed, the rest should fail with conflict or status errors
    assertEquals(1, successCount, "Expected exactly 1 successful RPC call, but got $successCount")
    assertEquals(
      concurrency - 1,
      errorCount,
      "Expected ${concurrency - 1} failed RPC calls, but got $errorCount"
    )
